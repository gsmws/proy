    ...
    {    
      ...
      {  
        ...
        {               
          ...
          $sql = $dbConn->prepare("SELECT * FROM alumno WHERE Id like '%$nombre%' OR Nombre like'%$nombre%' OR Usuario like'%$nombre%'");      
          $sql->execute();      
          header("HTTP/1.1 200 OK");      
        ...
        elseif ((isset($_GET['Nombre'])) & (isset($_GET['Password'])))
        {         
          $us=$_REQUEST['Nombre']; 
          $ps=$_REQUEST['Password'];
          $password= sha1($ps);
          $sql = $dbConn->prepare("SELECT * FROM alumno WHERE Usuario='$us' AND Password='$password' AND Nombre='AdministradorSF'");
          $sql->execute();
          header("HTTP/1.1 200 OK");            
          return $sql->fetch(PDO::FETCH_ASSOC);            
          exit();                                                
